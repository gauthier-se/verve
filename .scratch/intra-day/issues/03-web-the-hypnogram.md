...
Shipped. `web/src/lib/night.ts` with `night.test.ts`, `hooks/use-night.ts`,
`components/night-page.tsx`, the `/nights/$date` route, and the link from the
sleep Metric page's table.

Three things came out of the implementation that the spec did not anticipate:

- **The bands are divs, not a chart.** Recharts draws series against a
  categorical or numeric axis and has no interval mark; placing each stage as a
  percentage of the night's own span is a dozen lines, needs no measuring pass,
  and puts the arithmetic in a pure module the test runner reaches. The colours
  and labels come from `lib/breakdown.ts`, so this page and the stacked bar read
  the same night the same way.
- **Only the table rows link, not the chart bars.** The spec said both. A bar is
  drawn by `PanelChart`, which is shared by every Panel and knows nothing about
  which Metric is special; teaching it to navigate for one Metric is coupling
  the milestone does not need. The table row is a Night already keyed by its
  label, and it is where a person reading the numbers is standing.
- **The axis is labelled in UTC**, like every other bound Verve stores and
  prints. A night rendered in one zone and labelled in another would appear to
  have happened at a different time than its own figures say.
